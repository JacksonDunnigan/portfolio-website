...
const Navbar = () => {
  ...
  const [fontSize, setFontSize] = useState('3rem'); // Initial font size
  const [logoSize, setLogoSize] = useState('25rem'); // Initial logo size
  const [indentSize, setIndentSize] = useState('-8rem'); // Initial indent size
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Adjust font size
      const newFontSize = scrollTop === 0 ? '3rem' : '1.5rem'; // Change the font size based on scroll position
